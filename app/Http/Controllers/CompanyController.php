...
class CompanyController extends Controller
{
    ...
    {
        $last_company = CompareList::where('id', $request->id)->orderBy('compare_id', 'DESC')->first();
        if (!empty($last_company)) {
            $count = CompareList::where('id', $request->id)->count();
            if ($count == 2) {
                $company = CompareList::where('id', $request->id)->orderBy('compare_id', 'ASC')->first();
                $company->where('compare_id', $company->compare_id)->delete();
            }


            if ($request->company_id != $last_company->company_id) {
        ...
        return redirect('/comparelist/module/comparedisplaylist');
    ...
    function showcomparelist()
    {
        $data = CompareList::select('*')
            ->where('id', '=', Auth::User()->id)->with('user', 'company','job')->orderBy('compare_id', 'DESC')->limit(2)->get()->toArray();
